...
/**
 3 ème solution avec le même fonctionnement que la solution 2, en attribuant en premier les gamemasters qui ne sont formé qu'à une seule room
 */
const assignGameMastersToRooms = () => {
    ...
    while (attempt < MAX_RETRIES && !success) {
        ...
        // On commence par attribué les gamemasters qui ne sont formé qu'à une seule room
        gamemasters.forEach(gamemaster => {
            if (gamemaster.trained_rooms.length === 1) {
                assignedGameMasters.add(gamemaster);
                sessions.push({room: rooms.find(room => room.id === gamemaster.trained_rooms[0]).name, gamemaster: gamemaster.name})
            }
        });
        ...
        rooms.forEach(room => {
            ...
            if (chosenGamemaster) {
                ...
                sessions.push({room: room.name, gamemaster: chosenGamemaster.name});
            } else {
                return null;
            }
        });
